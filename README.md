...
**判定细则**（冷启动测试暴露过的模糊点，明文定下）

- 查判决：`shots/` 的目录位置就是判决——`keep/` 是认可、`maybe/` 待定、reject 不收录。
- 分数只在同类别内排序：**类别匹配优先于分数**。5 分的相机镜头不能顶替 4 分的开场镜头。
- 改编时长：等比压缩帧数允许，但每个改过的数字都要标 ASSUMPTION。
- 任务没指定设计系统时，先问 Yoru 用哪套，不要默认放飞。
- 任务类型在 `gold/` 没有母版时，按 [`gold/README.md`](gold/README.md) 的「空位规则」走：
  可以借 palette 和节奏包络，**构图必须作为 proposal 交回拍板，不许当成品交付**。
